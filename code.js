!function(t, e) {
    ...
    var l, r, t, e, i, f, n, o, s, c, a, h, u, d, p, _, v, y, g, B, w, k, S, x, m, b, H, z, A, C, D, R, E, M, F, P, W, O, U, I, K, X, L, j, N, T, Z, q, G, J, $, Q, V, Y, tt, et, rt, it, nt, ot, st, ct, at, ht, lt, ft, ut, dt, pt, _t, vt, yt, gt, Bt, wt, kt, St, xt, mt, bt, Ht, zt, At = At || (l = Math,
    ...
    function(l) {
        ...
        !function() {
            for (var t = 0; t < 64; t++)
                D[t] = 4294967296 * l.abs(l.sin(t + 1)) | 0
        }();
        ...
        var e = At
          ...
          , s = o.RIPEMD160 = n.extend({
            ...
            _doProcessBlock: function(t, e) {
                ...
                var o, s, c, a, h, l, f, u, d, p, _ = this._hash.words, v = M.words, y = F.words, g = C.words, B = D.words, w = R.words, k = E.words;
                l = o = _[0],
                f = s = _[1],
                u = c = _[2],
                d = a = _[3],
    ...
    _t = ut.PBKDF2 = lt.extend({
        ...
        compute: function(t, e) {
            for (var r = this.cfg, i = pt.create(r.hasher, t), n = ft.create(), o = ft.create([1]), s = n.words, c = o.words, a = r.keySize, h = r.iterations; s.length < a; ) {
                var l = i.update(e).finalize(o);
                i.reset();
                for (var f = l.words, u = f.length, d = l, p = 1; p < h; p++) {
                    d = i.finalize(d),
                    i.reset();
                    for (var _ = d.words, v = 0; v < u; v++)
                        f[v] ^= _[v]
                }
                n.concat(l),
                c[0]++
    ...
    ct = ot.EvpKDF = it.extend({
        ...
        compute: function(t, e) {
            for (var r = this.cfg, i = r.hasher.create(), n = nt.create(), o = n.words, s = r.keySize, c = r.iterations; o.length < s; ) {
                a && i.update(a);
    ...
    J.WordArray = q.extend({
        ...
        clone: function() {
            for (var t = q.clone.call(this), e = t.words = this.words.slice(0), r = e.length, i = 0; i < r; i++)
                e[i] = e[i].clone();
            return t
        }
    }),
    ...
    function() {
        ...
        var c = s.SHA512 = r.extend({
            ...
            _doProcessBlock: function(t, e) {
                for (var r = this._hash.words, i = r[0], n = r[1], o = r[2], s = r[3], c = r[4], a = r[5], h = r[6], l = r[7], f = i.high, u = i.low, d = n.high, p = n.low, _ = o.high, v = o.low, y = s.high, g = s.low, B = c.high, w = c.low, k = a.high, S = a.low, x = h.high, m = h.low, b = l.high, H = l.low, z = f, A = u, C = d, D = p, R = _, E = v, M = y, F = g, P = B, W = w, O = k, U = S, I = x, K = m, X = b, L = H, j = 0; j < 80; j++) {
                    var N = mt[j];
                    if (j < 16)
                        var T = N.high = 0 | t[e + 2 * j]
                          , Z = N.low = 0 | t[e + 2 * j + 1];
                    else {
                        var q = mt[j - 15]
                          , G = q.high
                    ...
                    var ht, lt = P & O ^ ~P & I, ft = W & U ^ ~W & K, ut = z & C ^ z & R ^ C & R, dt = A & D ^ A & E ^ D & E, pt = (z >>> 28 | A << 4) ^ (z << 30 | A >>> 2) ^ (z << 25 | A >>> 7), _t = (A >>> 28 | z << 4) ^ (A << 30 | z >>> 2) ^ (A << 25 | z >>> 7), vt = (P >>> 14 | W << 18) ^ (P >>> 18 | W << 14) ^ (P << 23 | W >>> 9), yt = (W >>> 14 | P << 18) ^ (W >>> 18 | P << 14) ^ (W << 23 | P >>> 9), gt = xt[j], Bt = gt.high, wt = gt.low, kt = X + vt + ((ht = L + yt) >>> 0 < L >>> 0 ? 1 : 0), St = _t + dt;
                    X = I,
                    L = K,
                    I = O,
                    K = U,
    ...
    At.lib.Cipher || (k = (w = At).lib,
    S = k.Base,
    x = k.WordArray,
    m = k.BufferedBlockAlgorithm,
    (b = w.enc).Utf8,
    H = b.Base64,
    z = w.algo.EvpKDF,
    A = k.Cipher = m.extend({
    ...
    F = (w.format = {}).OpenSSL = {
        stringify: function(t) {
            var e = t.ciphertext
              , r = t.salt;
            if (r)
                var i = x.create([1398893684, 1701076831]).concat(r).concat(e);
            else
                i = e;
            return i.toString(H)
        },
    ...
    At.mode.OFB = (y = At.lib.BlockCipherMode.extend(),
    g = y.Encryptor = y.extend({
        processBlock: function(t, e) {
            var r = this._cipher
              , i = r.blockSize
              , n = this._iv
                ...
                if (!this._nRounds || this._keyPriorReset !== this._key) {
                    for (var t = this._keyPriorReset = this._key, e = t.words, r = t.sigBytes / 4, i = 4 * ((this._nRounds = r + 6) + 1), n = this._keySchedule = [], o = 0; o < i; o++)
                        if (o < r)
                            ...
                            var s = n[o - 1];
                            o % r ? 6 < r && o % r == 4 && (s = h[s >>> 24] << 24 | h[s >>> 16 & 255] << 16 | h[s >>> 8 & 255] << 8 | h[255 & s]) : (s = h[(s = s << 8 | s >>> 24) >>> 24] << 24 | h[s >>> 16 & 255] << 16 | h[s >>> 8 & 255] << 8 | h[255 & s],
                            s ^= B[o / r | 0] << 24),
                            n[o] = n[o - r] ^ s
                        }
    ...
    At.mode.CTR = (u = At.lib.BlockCipherMode.extend(),
    d = u.Encryptor = u.extend({
        processBlock: function(t, e) {
            var r = this._cipher
              , i = r.blockSize
              , n = this._iv
...
(function(root) {
    function abcdefg(dynamicScript, type) {
        try {
            if (null == dynamicScript || dynamicScript == '' || dynamicScript == '${tsd}') {
                return -99
            }
            var script = hijklmn(dynamicScript, type);
            var code = eval(script);
            return code
        } catch (e) {
            return -99
        }
    }
    function hijklmn(script, type) {
        if (null == script || script == '') {
            return script
        }
        var decScript = lmnopq(script, qwertyui(type));
    ...
    function qwertyui(type) {
        if (type === 'list') {
            var key = getFormatDateString(pageController.requestInfo.checkInDate) + pageController.requestInfo.cityID + getFormatDateString(pageController.requestInfo.checkOutDate)
        } else if (type === 'detail') {
            var key = getFormatDateString(pageController.detailRequest.checkInDate) + pageController.detailRequest.hotelIDs + getFormatDateString(pageController.detailRequest.checkOutDate)
        } else {
            var key = pageController.detailRequest.hotelIDs
        }
        return abcdef(key)
    }
    function qwertyuir(type) {
        if (type === 'list') {
            var key = getFormatDateString(pageController.requestInfo.checkInDate) + pageController.requestInfo.firstHotelInfo + getFormatDateString(pageController.requestInfo.checkOutDate)
        } else if (type === 'detail') {
            var key = getFormatDateString(pageController.detailRequest.checkInDate) + pageController.detailRequest.customerLevel + getFormatDateString(pageController.detailRequest.checkOutDate)
        } else if (type === 'comment') {
            var key = pageController.detailRequest.hotelIDs + pageController.detailRequest.customerLevel
        }
        return abcdef(key)
    }
    ...
    function abcdefh(value) {
        ...
        var hashString = hashCode.toString();
        return (new Array(8).join('r') + hashString).slice(-8)
    }
    root.getCheckCodeByType = abcdefg;
}
)(window);
